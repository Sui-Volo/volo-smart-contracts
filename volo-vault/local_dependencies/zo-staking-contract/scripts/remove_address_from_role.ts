...
async function main() {
    const { env, gasBudget, roleName, userAddress } = await prompts([
        ...
        { type: 'text', name: 'userAddress', message: 'Enter the address to remove from the role' },
    ]);
    ...
    txb.moveCall({
        target: `${zoStaking.package}::admin::remove_address_from_role`,
        arguments: [
    ...
    try {
        console.log(`\n🚀 Removing address ${userAddress} from role "${roleName}" in ACL ${zoStaking.acl_control}...`);
        ...
        if (result.effects?.status.status === 'success') {
            console.log('✅ Successfully removed address from role.');
        } else {
